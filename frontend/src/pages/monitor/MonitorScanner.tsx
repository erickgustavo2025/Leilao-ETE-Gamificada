...
export function MonitorScanner() {
    ...
    const validateMutation = useMutation({
        mutationFn: async (code: string) => {
            return await api.post('/tickets/validate', { hash: code });
        },
        onSuccess: (response) => {
            setResult({ success: true, data: response.data.ticket });
            playSuccess();
                    ...
                    {isCamOpen ? (
                        <div className="w-full aspect-square max-w-[300px] bg-black border-4 border-yellow-500 rounded-lg overflow-hidden relative shadow-[0_0_20px_rgba(234,179,8,0.3)]">
                            ...
                            <button 
                                ...
                                className="absolute bottom-2 right-2 bg-red-600 p-2 rounded text-white z-20 hover:bg-red-500 transition-colors"
                            >
                                <CameraOff size={20}/>
                            </button>
                        </div>
                        ...
                        <PixelButton 
                            ...
                            className="w-full py-8 bg-slate-800 border-dashed border-2 border-slate-600 text-slate-400 hover:text-white flex flex-col items-center gap-2 group transition-all hover:border-yellow-500 hover:bg-slate-700"
                        >
